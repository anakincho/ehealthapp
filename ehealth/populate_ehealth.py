...
def populate():
    add_user('joe', 'bobkennedy@hotmail.co.uk', 'joe')

    add_user('jill', 'jillvalentine@hotmail.co.uk', 'jill')

    add_user('jim', 'jim@abv.bg', 'jim')

    diabetes_cat = add_cat('Diabetes', 'jill', False)  # creates a category with name Diabetes, by the user testUser
    ...
    flu_cat = add_cat('Flu', 'jill', True)  # creates a category with name Flu, by the user testUser
...
def add_page(cat, title, url, flesch_score, polarity_score, subjectivity_score, shared,user):
